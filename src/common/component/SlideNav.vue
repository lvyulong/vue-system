          ...
          <template v-for="(child,childIndex) in config.children">
            ...
            <el-menu-item-group v-if="child.name">
              ...
              <template v-for="(item,itemIndex) in child.items">
                <el-menu-item :index="item.path" style="padding-left: 52px">{{item.label}}</el-menu-item>
              </template>
            </el-menu-item-group>
            <!-- 无分组 -->
            <el-menu-item :index="child.path" v-if="!child.name" style="padding-left: 52px">{{child.label}}
            </el-menu-item>
          </template>
        </el-submenu>
        <!-- 无二级菜单 -->
        <el-menu-item :index="config.path" v-if="!config.children">
          <i :class="config.icon" v-if="config.icon"></i>
